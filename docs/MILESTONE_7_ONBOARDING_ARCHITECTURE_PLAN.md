...
- **Phase 7.2A.2 (Final Pre-Deployment Language-Preference Hardening — Local/CI Only)**: COMPLETE (PASS) — Strictly user-owned `preferred_language` enforced by `private.validate_profile_preferred_language_update()` BEFORE UPDATE trigger on `public.profiles` (admin updating another user's preference rejected with 42501; admin updates to other columns preserved; active user self-update and trusted server operations permitted); post-signout state synchronization in `I18nProvider` ensuring authenticated profile preference persists into subsequent signed-out sessions without render loops; regression test suites (25 pgTAP assertions, 224 frontend tests) 100% PASS.
- **Phase 7.2A.3 (Controlled Hosted Bilingual Foundation Deployment)**: COMPLETE (PASS) — Applied migration `20260924010000_milestone_7_bilingual_foundation.sql` to linked hosted Supabase `zlaixhnyydxgbphgsetv` (31 total hosted migrations); verified `profiles.preferred_language` (NOT NULL default `'en'`, CHECK `'en'|'ms'`, backfilled for 3 existing profiles); verified `profiles_validate_preferred_language_update` trigger and `private.validate_profile_preferred_language_update()` active; verified `courses.title_ms`/`description_ms` and `cohorts.name_ms`/`description_ms` (NULL for existing rows, check constraints 2–160 chars); verified `public.resource_language` enum and `resources.content_language` (backfilled to `'en'` for 8 existing resources); verified 100% TypeScript type parity; verified 0 security/performance advisor issues attributable to Phase 7.2; defensive 42703 adapter intentionally retained for Phase 7.2B cleanup.
- **Phase 7.2B (Production Non-Assessment Localization & Localized Metadata)**: COMPLETE (PASS) — Decommissioned temporary 42703 adapter in `account-access-repository.ts` and removed `preferredLanguageAvailable` client flag; direct queries to `profiles.preferred_language`; added bilingual cohort authoring (`nameMs`, `descriptionMs`) and rendering across Operations (People, Cohorts Admin, My Cohorts); implemented `ResourceLanguage` enum classification throughout live resource catalog, admin authoring, and library filters (verifying "All languages" displays all resources regardless of UI locale); localized all non-assessment production workflows (People, Cohorts, Guides, Teaching Kit, Protected PDF Viewer, Resources Admin, Resource Detail, Resource Taxonomy, Resource Versions); active locale-sensitive course title search and display; centralized typed enum label helpers (`src/lib/i18n/enum-labels.ts`); accessible date/time formatting with `formatDateTime`/`formatDate`; 100% dictionary parity between EN and MS; 41 test files (235 tests) pass, typecheck 0 errors, lint 0 errors, build succeeds; 15 local db test suites (465 tests) pass; ZERO new database migrations (count stays 31), ZERO hosted schema/data mutations.
- **Phase 7.2B.1 (Non-Assessment Localization Completion Pass)**: COMPLETE (PASS) — Locked resource content-language filtering semantics across all 5 options (`all`, `en`, `ms`, `bilingual`, `language_independent`); extracted and verified `normalizeLanguageParam(raw)` URL query normalizer; dynamic `{param}` token interpolation in `t(key, params)` for non-word-order-sensitive translations; completed comprehensive localization of remaining hardcoded strings in targeted non-assessment files (eyebrows, notices, dialogs, labels, fallbacks, legends, watermarks, and aria-labels); enforced locale-neutral data repositories (`resource-admin-repository` and `resource-taxonomy-repository`) with presentation fallback resolution and non-translated authored names in UI; removed duplicate label sources (`resourceStatusLabels` and `resourceTypeLabels` eliminated); verified date/time formatting in non-assessment flows respects application locale (zero `Intl.DateTimeFormat(undefined)`); maintained 100% dictionary key parity between `en.ts` and `ms.ts` (524 keys each); 41 test files (244 tests) pass, typecheck 0 errors, lint 0 errors, build succeeds; 15 local db test suites (465 tests) pass; ZERO new database migrations (count stays 31), ZERO hosted schema/data mutations.
...
- **Phase 7.2C (Assessment & Content Localization)**: PLANNED (NOT STARTED) — Bilingual quiz question and option schema (`prompt_ms`, `option_text_ms`) with immutable bilingual attempt snapshots; learner quiz payload language selection; admin bilingual quiz authoring and editors; item analysis and results localization; CSV exports localization; and cohort readiness bilingual views.
