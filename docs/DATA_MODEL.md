...
| `preferred_language` | text | `en` (default/fallback) or `ms`. Strictly user-owned (enforced by `private.validate_profile_preferred_language_update()` trigger). In client runtime, availability is explicitly signaled via `preferredLanguageAvailable: boolean` to distinguish unmigrated Phase 7.1 environments from explicit English choice. |
