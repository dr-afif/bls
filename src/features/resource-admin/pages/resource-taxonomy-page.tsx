...
function TaxonomySection({
  ...
  const label = kind === "topic" ? t("resourceAdmin.taxonomy.topics") : t("resourceAdmin.taxonomy.stages");
  ...
  const resourceCount = (count: number) => `${count} ${count === 1 ? t("resource.library.resource") : t("resource.library.resources")}`;
    ...
    <Card className="min-w-0">
      <CardHeader>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <CardTitle>
            <Icon aria-hidden="true" className="mr-2 inline size-5" />
            {label}
          </CardTitle>
          <Badge variant="neutral">
            {items.filter((item) => item.active).length} {t("resourceAdmin.taxonomy.active")} · {items.length} {t("resourceAdmin.resources.total")}
          </Badge>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        <p className="text-sm text-muted-foreground">
          {kind === "topic"
            ? "Clinical discovery labels used across learner and instructor resources."
            : "Instructor teaching-kit labels for moments such as preparation, demonstration, and debrief."}
        </p>
        {!adding ? (
          <Button onClick={() => setAdding(true)} variant="outline">
            <Plus aria-hidden="true" />
            {kind === "topic" ? t("resourceAdmin.taxonomy.addTopic") : t("resourceAdmin.taxonomy.addStage")}
          </Button>
        ) : (
          <TaxonomyItemForm busy={busy} kind={kind} onCancel={() => setAdding(false)} onSubmit={create} />
        )}
        {items.length === 0 ? (
          <StatePanel compact kind="empty" title={`No ${label.toLowerCase()} yet`} description="Add the first label for this organization." />
        ) : (
          <ol className="space-y-3">
            {items.map((item) => (
              <li className="rounded-xl border p-4" key={item.id}>
                {editingId === item.id ? (
                  <TaxonomyItemForm busy={busy} item={item} kind={kind} onCancel={() => setEditingId(null)} onSubmit={(values) => update(item, values)} />
                ) : (
                  <>
                    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          {item.active ? (
                            <Badge variant="success">
                              ...
                              {t("resourceAdmin.taxonomy.active")}
                            </Badge>
                          ) : (
                            <Badge variant="neutral">
                              <ToggleLeft aria-hidden="true" className="size-3.5" />
                              {t("resourceAdmin.taxonomy.inactive")}
                            </Badge>
                          )}
                          <Badge variant="neutral">
                            {t("resourceAdmin.taxonomy.order")} {item.displayOrder}
                          </Badge>
                        </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        <Button disabled={busy} onClick={() => setEditingId(item.id)} size="sm" variant="outline">
                          <Pencil aria-hidden="true" />
                          {t("resourceAdmin.taxonomy.edit")}
                        </Button>
                        <Button
                          disabled={busy || (item.active && item.blockingPublishedResourceCount > 0)}
                          onClick={() => void toggle(item)}
                          size="sm"
                          variant={item.active ? "ghost" : "outline"}
                        >
                          <ToggleLeft aria-hidden="true" />
                          {item.active ? t("resourceAdmin.taxonomy.deactivate") : t("resourceAdmin.taxonomy.activate")}
                        </Button>
                      </div>
                    </div>
                    <dl className="mt-4 grid gap-3 border-t pt-3 text-sm sm:grid-cols-3">
                      <div>
                        <dt className="font-semibold">{t("resourceAdmin.taxonomy.allAssignments")}</dt>
                        <dd className="text-muted-foreground">{resourceCount(item.usageCount)}</dd>
                      </div>
                      <div>
                        <dt className="font-semibold">{t("resourceAdmin.taxonomy.publishedUse")}</dt>
                        <dd className="text-muted-foreground">{resourceCount(item.publishedUsageCount)}</dd>
                      </div>
                      <div>
                        <dt className="font-semibold">{t("resourceAdmin.taxonomy.blockingPublication")}</dt>
                        <dd className="text-muted-foreground">{resourceCount(item.blockingPublishedResourceCount)}</dd>
                      </div>
                    </dl>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            ...
            {t("resourceAdmin.taxonomy.recentActivity")}
          </CardTitle>
        </CardHeader>
        <CardContent>
