...
export function ResourcesAdminPage() {
  ...
  return <div className="space-y-6">
    <PageHeader action={<Button asChild><Link to="/app/admin/resources/new"><FilePlus2 aria-hidden="true" />New resource</Link></Button>} description="Create, review, approve, publish, replace, and retire course resources. Lifecycle decisions are server-enforced and audit recorded." eyebrow="Live development workspace" title="Resources" />
    <section aria-label="Resource filters" className="grid gap-3 rounded-2xl border bg-card p-4 shadow-sm md:grid-cols-2 xl:grid-cols-3">
      ...
      <label className="text-sm font-semibold">BLS topic<select className={selectClassName} onChange={(event) => setFilter("topic", event.target.value)} value={topic}><option value="all">All topics</option>{catalog.data?.topics.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      <label className="text-sm font-semibold">Teaching stage<select className={selectClassName} onChange={(event) => setFilter("stage", event.target.value)} value={stage}><option value="all">All stages</option>{catalog.data?.stages.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
