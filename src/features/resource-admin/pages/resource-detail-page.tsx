    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(18rem,1fr)]">
      <div className="space-y-6">
        ...
        <Card><CardHeader><CardTitle>Audience and discovery</CardTitle></CardHeader><CardContent><form className="grid gap-5 lg:grid-cols-3" onSubmit={updateClassifications}>
          ...
          <fieldset><legend className="text-sm font-semibold">BLS topics</legend><div className="mt-2 max-h-56 space-y-2 overflow-y-auto">{catalog.data?.topics.map((topic) => <label className="flex min-h-11 items-center gap-3 rounded-xl border px-3 text-sm" key={topic.id}><input className="size-5" defaultChecked={resource.topics.some((item) => item.id === topic.id)} name="topicIds" type="checkbox" value={topic.id} />{topic.name}</label>)}</div></fieldset>
          <fieldset><legend className="text-sm font-semibold">Teaching stages</legend><div className="mt-2 max-h-56 space-y-2 overflow-y-auto">{catalog.data?.stages.map((stage) => <label className="flex min-h-11 items-center gap-3 rounded-xl border px-3 text-sm" key={stage.id}><input className="size-5" defaultChecked={resource.stages.some((item) => item.id === stage.id)} name="stageIds" type="checkbox" value={stage.id} />{stage.name}</label>)}</div></fieldset>
