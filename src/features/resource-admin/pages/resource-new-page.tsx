  ...
  return <div className="space-y-6">
    ...
    <form className="space-y-6" onSubmit={submit}>
      ...
      <Card><CardHeader><CardTitle>Audience and discovery</CardTitle></CardHeader><CardContent className="grid gap-5 lg:grid-cols-3">
        ...
        <fieldset><legend className="text-sm font-semibold">BLS topics</legend><div className="mt-2 max-h-56 space-y-2 overflow-y-auto">{catalog.data?.topics.map((topic) => <label className="flex min-h-11 items-center gap-3 rounded-xl border px-3 text-sm" key={topic.id}><input className="size-5" type="checkbox" value={topic.id} {...form.register("topicIds")} />{topic.name}</label>)}</div><FieldError>{form.formState.errors.topicIds?.message}</FieldError></fieldset>
        <fieldset><legend className="text-sm font-semibold">Teaching stages</legend><div className="mt-2 max-h-56 space-y-2 overflow-y-auto">{catalog.data?.stages.map((stage) => <label className="flex min-h-11 items-center gap-3 rounded-xl border px-3 text-sm" key={stage.id}><input className="size-5" type="checkbox" value={stage.id} {...form.register("stageIds")} />{stage.name}</label>)}</div><FieldError>{form.formState.errors.stageIds?.message}</FieldError></fieldset>
      </CardContent></Card>
