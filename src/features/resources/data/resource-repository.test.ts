...
describe("assembleResourceCatalog", () => {
  it("assembles immutable resource versions with ordered taxonomy and relations", () => {
    const resources = assembleResourceCatalog({
      resources: [{ id: "resource-1", current_version_id: "version-1", slug: "scene-safety", title: "Old title", resource_type: "checklist", estimated_minutes: 2, featured: true, content_language: "ms" }],
  ...
  it("fails closed when a structured snapshot is malformed", () => {
    expect(() => assembleResourceCatalog({
      resources: [{ id: "resource-1", current_version_id: "version-1", slug: "guide", title: "Guide", resource_type: "guide", estimated_minutes: null, featured: false, content_language: "en" }],
